window.addEventListener("load", function(){
    ...
    formulario.addEventListener("submit", function(e){
        if (nombreProd.value == "" || nombreProd.value.length < 5 || precio.value == "" || precio.value < 0 || categoria.value == "" || talle.value == "" || descripcion.value == "" || descripcion.value < 20 || !imagen.files.length){
            e.preventDefault();            
        }
    });
    ...
    imagen.addEventListener("blur",function(e){
        document.querySelector("#error_imagen").style.display ="none";
        if (!imagen.files.length){
            document.querySelector("#error_imagen").style.display ="block";
            return
        }else{
            document.querySelector("#error_imagen").style.display ="none";
        }
